...
**Case Document**
A file (PDF or DOCX) uploaded as evidence and linked to a Case. Uploaded either during Create Case (before the Case exists — linked to it once created) or, going forward, from within that Case's own chat. Shortly after upload it becomes something the Case's chat can search and reference when answering — see `docs/adr/0008-case-document-rag-for-chat.md`. Distinct from a Case Law Document (an ingested case/statute record from an external corpus) and a Generated Article (AI-written Library content) — none of these three are the same kind of "document."
_Avoid_: "document" alone (ambiguous with Case Law Document and Generated Article — always say Case Document for an uploaded evidentiary file), "attachment" (this app's chat/upload UI copy doesn't use that word)
