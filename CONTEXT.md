...
- Case: `POST /api/my-cases` accepts only `caseName`/`partyInvolved` (single string)/`notes` — it has no `actionType`, `jurisdiction`, or structured Party (name + designation) fields. Spec for the needed columns/`Party` model has been handed off to the backend team. The Create Case page now submits against the real endpoint (`caseTitle`→`caseName`, Parties collapsed into a single `partyInvolved` string, documents uploaded via the real `POST /api/documents` then linked with `PATCH /api/documents/:id` once the case id is known) — it works end-to-end today, but Type of Action and Jurisdiction are captured in the form and silently not persisted until the backend ships them (the form shows an inline notice saying so).
- Case Chat — a Case will support multiple Conversations (one-to-many), with the Case's structured fields injected into the AI's context so it doesn't need re-explaining. Spec (nullable `Conversation.caseId` FK, context-injection mechanism) has been handed off to the backend team; blocked on that plus the Case field reconciliation above (little to inject until `actionType`/`jurisdiction`/Party exist). Case Portfolio (list) and a Case detail page (`/homepage/case-portfolio/[id]`) are wired to the real `GET /api/my-cases` / `GET /api/my-cases/:id` today as the foundation this will attach to. Document content (text-extraction/RAG) is explicitly out of scope for this feature.
