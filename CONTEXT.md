...
- Legal Terminal auto-refresh — **proposed, not built.** After documents finish indexing, contradictions and case strategy already run (45s quiet window), but findings (Legal Issues / Strengths / Weaknesses / Attack / Defense) only fill when the lawyer clicks Refresh analysis. Intent: coalesce corpus changes into the same `caseRefresh` job as that button, keep lawyer-typed rows, do not auto Red Team / mind map / audio, and only auto-generate reconstruction when none exists yet. Pickup spec: `ilovelawyer-api/docs/legal-terminal-auto-refresh.md`. Product ADR: `docs/adr/0014-legal-terminal-auto-refresh.md`. Issues: [API epic #73](https://github.com/FORHU/ilovelawyer-api/issues/73), [app #110](https://github.com/FORHU/ilovelawyer-app/issues/110).
