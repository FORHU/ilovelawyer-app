...
export default function ArchiveDocumentModal({ doc, isArchiving, onConfirm, onClose }: ArchiveDocumentModalProps) {
      ...
      <DialogContent showCloseButton={false} className="max-w-md gap-0 overflow-hidden p-0">
        ...
        <div className="px-6 py-6 flex gap-4">
          ...
          <DialogDescription asChild>
            <p className="text-sm text-foreground leading-relaxed">
              {t("detail.archiveDocumentConfirm", { documentName: doc.name })}
            </p>
          </DialogDescription>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border bg-muted/40">
          <Tooltip>
            ...
            <TooltipContent>Keep this document active and close without archiving</TooltipContent>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger asChild>
              <button
              ...
              >
                ...
                {isArchiving ? t("archiving") : t("detail.archiveDocumentCta")}
              </button>
            </TooltipTrigger>
            <TooltipContent>Move {doc.name} to the Archived view — you can restore it anytime</TooltipContent>
          </Tooltip>
        </div>
      </DialogContent>
    </Dialog>
