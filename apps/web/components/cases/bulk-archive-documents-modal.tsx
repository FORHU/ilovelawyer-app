...
interface BulkArchiveDocumentsModalProps {
  count: number;
  isArchiving: boolean;
  onConfirm: () => void;
  onClose: () => void;
...
/** Confirms a multi-document archive (Select All / bulk selection in DocumentFolderBrowser).
 * Same chrome as ArchiveDocumentModal, generalized to a count instead of a single doc name —
 * archiving a "folder" resolves to this too, since folders are just a derived grouping and
 * archiving one really means archiving every document currently in it. */
export default function BulkArchiveDocumentsModal({ count, isArchiving, onConfirm, onClose }: BulkArchiveDocumentsModalProps) {
      ...
      <DialogContent showCloseButton={false} className="max-w-md gap-0 overflow-hidden p-0">
        <div className="flex items-center justify-between px-6 py-5 border-b border-border bg-muted/60">
          <DialogTitle asChild>
            <h2 className="font-['Libre_Caslon_Text'] text-lg text-foreground font-normal">
              {t("detail.archiveDocumentsCta", { count })}
            </h2>
          </DialogTitle>
          <Tooltip>
        ...
        <div className="px-6 py-6 flex gap-4">
          ...
          <DialogDescription asChild>
            <p className="text-sm text-foreground leading-relaxed">{t("detail.archiveDocumentsConfirm", { count })}</p>
          </DialogDescription>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border bg-muted/40">
          <Tooltip>
            ...
            <TooltipContent>{t("detail.keepDocumentsActiveTooltip")}</TooltipContent>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger asChild>
              <button
            ...
            <TooltipContent>{t("detail.archiveDocumentsTooltip", { count })}</TooltipContent>
          </Tooltip>
        </div>
      </DialogContent>
    </Dialog>
