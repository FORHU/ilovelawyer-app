...
export default function ConsultationChat({
  ...
  //
  // Must wait for caseConsultations (and, once autoSelectedRef above has redirected onto an
  // existing one, that consultation's own history) to actually load before deciding there's
  // nothing to generate onto yet. Firing as soon as `session` is ready — before either of
  // those queries resolve — raced the auto-select-most-recent effect above: on every fresh
  // isolated mount (e.g. Terminal's Mind Map pane) `activeMindMap` reads as false against an
  // still-empty transcript, so this created a brand-new consultation and immediately sent the
  // prompt into it instead of reusing the case's existing one — leaving a fresh "Visual
  // Strategy Map"-titled duplicate behind on every mount.
  ...
  useEffect(() => {
    ...
    if (caseConsultations === undefined) return; // still loading — don't assume "none yet"
    if (!consultationId && caseConsultations.length > 0) return; // auto-select is about to pick one
    if (consultationId && history === undefined) return; // existing thread's history not loaded yet
    // A generation kicked off from another tab (or this one, before a refresh) is already
    // running — don't also fire from here and hit the backend's duplicate-generation guard.
    if (mindMapJob.data?.status === "IN_PROGRESS") return;
    autoGeneratedMindMapRef.current = true;
    void doSend(AUTO_MINDMAP_PROMPT);
  }, [embedded, mindMapOnly, caseId, activeTab, activeMindMap, isSending, session, caseConsultations, consultationId, history, mindMapJob.data?.status]);
